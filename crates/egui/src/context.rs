...
impl Context {
    ...
    pub fn shadow_painter<R>(&self, reader: impl FnOnce(&ShadowPainter) -> R) -> R {
        self.read(|ctx| reader(&ctx.shadow_painter))
    }

    pub fn shadow_painter_mut<R>(&self, writer: impl FnOnce(&mut ShadowPainter) -> R) -> R {
        self.write(|ctx| writer(&mut ctx.shadow_painter))
    }
}
